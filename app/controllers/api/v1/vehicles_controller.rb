...
class Api::V1::VehiclesController < Api::V1::BaseController
  ...
  def index
    respond_with sub_account.memberships.joins("inner join investment_assets on investment_assets.id = memberships.investment_asset_id").where("investment_assets.asset_type in (8)").active.without_custom_asset,
                 movements: true, root: "vehicles"
  end
