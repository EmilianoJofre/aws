class Api::V1::DemoInvestmentAssetSerializer < Api::V1::BaseSerializer
  attributes :id, :currency, :asset_id, :name, :asset_type, :sub_sector, :average_annual_cost,
             :mtd, :recovery_level, :qtd, :ytd, :y1, :y3, :y5
  ...
  def memberships
    @memberships ||= relation.demo_memberships.where(investment_asset: object)
  end
  ...
  def membership_balance(membership)
    membership.amount_updated_balance
  end

  def membership_original(id)
    Membership.find(id)
  end
end
