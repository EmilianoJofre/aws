...
export default {
  ...
  data() {
    return {
      ...
      accumulatedHeaders: [
        'amountInvestedBalanceRe',
        'incomesBalanceRe',
        'incomesPercentageRe',
      ],
      headersOrder: {
        amountInvestedBalanceRe: 0,
      },
      selectedAccount: 'all',
      selectedTab: 'accounts',
      selectedColumn: 'amountInvestedBalanceRe',
  ...
  methods: {
    ...
    changeTab(key) {
      ...
      this.selectedColumn = 'amountInvestedBalanceRe';
    ...
    setTextColor(account) {
      if (this.selectedColumn === 'incomesBalanceRe' || this.selectedColumn === 'incomesPercentageRe') {
    ...
    reduceColumn(account) {
      ...
      if (this.selectedColumn === 'incomesPercentageRe') {
        const totalAmountInvested = subAccounts.reduce((acc, curr) =>
          acc + this.reduceMembershipsByColumn(curr, 'amountInvestedBalanceRe'), 0,
        );

        const weightedPercentageSum = subAccounts.reduce((acc, curr) =>
          acc +
          this.reduceMembershipsByColumn(curr, 'amountInvestedBalanceRe') *
          this.reduceMembershipsByColumn(curr, 'incomesPercentageRe'), 0,
        );
    ...
    reduceMembershipsByColumn(subAccount, column) {
      ...
      if (column === 'incomesPercentageRe') {

        return sumprod / totalAmountInvested;
      } else if (column === 'incomesBalanceRe') {
      ...
      if(this.selectedTab === 'accounts') { 
        if(this.$store.state.currency.currency !== currency) {
          if(currency === 'CLP') {
            ...
            return this.transformValueToDollar(amount);
          }
          ...
          return this.transformValueToClp(amount);
        } 
      };
