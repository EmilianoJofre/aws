...
export default {
  ...
  computed: {
    membership() {
      return this.$store.getters.selectedPensionFund(this.membershipType);
    },
    movements() {
  ...
  methods: {
    createMovement(type) {
      this.$modal.show(moneyMovementCreateModal,
        { movementType: type, selectedId: this.selectedPensionFundId },
        { height: 'auto' },
    ...
    beforeClose() {
      if (this.membershipType === 'normal') {
        ...
        this.$store.dispatch('getCustomPensionFunds');
      }
    },
    liquidateMembership() {
      this.loading = true;
      PensionFundsApi.liquidatePensionFunds(this.membership)
