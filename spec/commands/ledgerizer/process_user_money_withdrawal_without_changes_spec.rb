...
describe Ledgerizer::ProcessUserMoneyWithdrawalWithoutChanges do
  ...
  let(:expected_entry) do
    {
      entry_code: :money_withdrawal_without_changes,
      entry_time: now,
  ...
  context "with valid data" do
    let(:currency) { 'USD' }
    let(:quotas) { 10 }
    let(:average_price) { 10 }
    let(:quotas_average_price) { 10 }
    let(:expected_amount_1) {  Money.from_amount(-100, currency) }
    let(:expected_amount_2) {  Money.from_amount(-(quotas * average_price), currency) }
    let(:expected_amount_3) {  Money.from_amount(-100, currency) }
    let(:expected_amount_4) {  Money.from_amount(-100, currency) }
