...
describe Ledgerizer::RevertUserMoneyWithdrawalWithExpenses do
  ...
  let(:expected_entry) do
    {
      entry_code: :revert_money_withdrawal_with_expenses,
      entry_time: now,
  ...
  context "with valid data" do
    let(:currency) { 'USD' }
    let(:quotas) { 5980 }
    let(:average_price) { 14.1483 }
    let(:quotas_average_price) { 14.2712 }
    let(:invested_amount) { (quotas_average_price * quotas).round(2) }
    let(:extra_amount) { (average_price * quotas).round(2) }
    let(:expected_amount_1) { Money.from_amount(invested_amount, currency) }
    let(:expected_amount_2) do
      Money.from_amount((extra_amount - invested_amount).round(2), currency)
    end
    let(:expected_amount_3) { Money.from_amount(extra_amount, currency) }
