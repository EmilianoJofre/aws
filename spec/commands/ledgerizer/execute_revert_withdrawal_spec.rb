...
describe Ledgerizer::ExecuteRevertWithdrawal do
  ...
  before do
    ...
    allow(Ledgerizer::RevertUserMoneyWithdrawalWithIncomes).to receive(:for)
    allow(Ledgerizer::RevertUserMoneyWithdrawalWithExpenses).to receive(:for)
  ...
  context "with sale price higher than average price" do
    ...
    it "calls correct commands" do
      perform
      expect(Ledgerizer::RevertUserMoneyWithdrawalWithIncomes).to have_received(:for).once.with(
        user: user,
        money_movement: money_movement
      )
      expect(Ledgerizer::RevertUserMoneyWithdrawalWithExpenses).not_to have_received(:for)
      expect(Ledgerizer::ProcessUserMoneyInvestment).not_to have_received(:for)
    end
  end
  ...
  context "with sale price lower than average price" do
    ...
    it "calls correct commands" do
      perform
      expect(Ledgerizer::RevertUserMoneyWithdrawalWithExpenses).to have_received(:for).once.with(
        user: user,
        money_movement: money_movement
      )
      expect(Ledgerizer::RevertUserMoneyWithdrawalWithIncomes).not_to have_received(:for)
      expect(Ledgerizer::ProcessUserMoneyInvestment).not_to have_received(:for)
    end
  end
